...
| ✅ | 多人 schema：`users` 表 ＋ 五個 CASCADE 外鍵，憑證 per-user 且加密儲存 |
| ✅ | 圖文選單三顆按鈕，固定字串走快速路徑不經過 LLM |
| ✅ | 相對日期由程式算好寫進 prompt，並保留星期驗證當安全網 |
...
### 32. 憑證放錯地方，比放錯人手上更難發現

**決策**：建 `users` 表當所有使用者資料的錨點，並把 refresh token 與 calendarId
從環境變數搬進加密後的 `google_credentials`。五張表加外鍵指向 `users`，全部 CASCADE。
...
| 事實 | 原本放哪 |
|---|---|
| 有沒有封鎖 | `blocked_users` 表 |
| 能不能直接匯入 | `CALENDAR_OAUTH_USER_IDS` |
| 他的 refresh token | `GOOGLE_REFRESH_TOKEN` |
| 寫進哪本行事曆 | `GOOGLE_CALENDAR_ID` |

三個是環境變數。而**環境變數天生只能有一份**。

#### 那個限制不是產品決定

`CalendarProperties` 的註解老老實實寫著：

> 憑證只有一份，所以這份名單實務上就是「擁有者」。名單放兩個人，
> 等於第二個人的行程會寫進第一個人的行事曆。

那句話是對的，但它把**症狀**寫成了**規格**。「只有一個人能直接匯入」不是產品上的
取捨，是「token 存在一個只能有一份的地方」的必然結果。搬進資料庫之後它自己消失。

而它藏得很深的原因跟決策 31 一樣：**單人使用時，錯的設計跟對的設計行為完全一致。**

#### 一個分類錯誤

四個 Google 設定看起來是同一類東西，其實是兩類：

| | 是什麼 | 該放哪 |
|---|---|---|
| `client_id` / `client_secret` | **應用程式**的身分，一份 | 環境變數 ✅ |
| `refresh_token` / `calendar_id` | **使用者**的憑證，每人一份 | 資料庫 |

`GoogleTokenProvider` 把這個錯誤具體化成一個欄位：`private String cachedToken`。
**一顆 token，所有人共用。**

#### 為什麼加密，以及它擋得住什麼

擋的是資料庫備份外洩、唯讀副本、以及**開發者自己跑 psql**——
這個專案的開發過程中 psql 跑過幾十次，明文 token 會留在終端機捲軸裡。
**擋不住**應用程式被攻破：金鑰就在那個行程裡。

所以它不是萬靈丹，是讓「DB 外流」不等於「所有人的行事曆寫入權外流」。

而加密只保護資料庫。保護其他地方的是另一行：

```java
public record GoogleAuthorization(String refreshToken, String calendarId, String scope) {
    @Override
    public String toString() { ... "refreshToken=(已隱藏)"; }
}
```

record 會自動生成一個把每個欄位都印出來的 `toString`，而第一個欄位是 refresh token。
任何一句 `log.debug("授權={}", auth)` 都會把行事曆的永久寫入權寫進 log 檔——
**那正是加密要防的那份外洩，只是換了個檔案。**
...
在它之前，「刪掉這個使用者的所有資料」是「記得掃五張表，而且要照對的順序」。
漏掉一張的後果不是報錯，是**留下一堆查不到主人的孤兒列**——
而那正是使用者要求刪除資料時最不想發生的事。

#### 差點多建五個索引

寫遷移時我順手加了五個 `CREATE INDEX`（外鍵不會自動建，而 CASCADE 要用）。
查了現況才發現：**四張表已經有以 `line_user_id` 開頭的複合索引**，前導欄位就夠用；
而 `idx_command_executions_user` 這個名字 V12 已經用掉了——直接 CREATE 會撞名，
**讓整個遷移因為一個跟這次改動完全無關的理由失敗**。真正缺的只有 `message_anchors`。

#### 環境變數變成種子

per-user 的授權流程（讓使用者在 LINE 裡自己跑完 OAuth）還沒做。只建表不填的話，
那張表會空到那天為止——**而空的表沒有人會發現欄位設計錯了**。

所以 `CalendarCredentialsBootstrap` 在啟動時把 `.env` 那組種進去：
**環境變數是種子，程式碼只讀表。** 讀取路徑從第一天就在跑真的資料。

寫入規則刻意是「不一樣才寫」而不是每次覆蓋：一般重啟完全安靜，
換過 token（例如 Testing 模式七天到期後重跑腳本）才寫入並大聲說——
因為那正是你想確認有沒有生效的時刻。

#### 這次沒做的

- **時區欄位**——加一個可為 NULL 的欄位隨時都行，貴的是那六處寫死 `Asia/Taipei`
  的程式碼。「以後加欄位很難」是個假理由，真正難改的是關聯與鍵。
- **Row-Level Security**——要先拆 DB 角色（現在這個 `rolbypassrls = t`），
  而且幾乎所有寫入都在背景 handler 裡、沒有「當前使用者」可設。得先造出那個上下文。
- **多租戶的授權端點**——那是功能不是 schema。但表就緒之後，只剩一個端點要寫。

#### 外鍵讓八支測試變紅，而那是對的

它們直接塞資料進去，跳過了 webhook 入口。正式環境保證使用者列先存在的是
`LineWebhookController.dispatch` 的第一行，測試繞過了它就得自己建——
**那不是測試的雜訊，是真實的寫入順序。**

### 33. 讓模型去理解一個我們自己寫死的字

**決策**：加圖文選單（三顆按鈕，後台建），並在指令解析前加一條
「完全相符就跳過 LLM」的快速路徑。

#### 選單的限制跟安全邊界剛好重合

圖文選單的按鈕**沒辦法引用某一則訊息**。而這個產品最大的坑正是這個
（決策 29／31）：不引用而直接打字，編號會對到另一份清單，
「第二筆」指到別的東西，**而且會確實地執行**。

所以選單**在結構上只做得了不帶編號的全域指令**——它做不到「刪掉第二筆」，
而那正是危險的那一類。

#### 按鈕送的是固定字串，卻要模型重新猜一次

`NoteCommandParser` 對**每一則**文字訊息都打一次 LLM。按鈕送的是
永遠一模一樣的字串，所以那條路有三個代價，而且都是真的：

- **慢**——Ollama 一次呼叫
- **不確定**——同一句每次重新猜一次意圖
- **打在瓶頸上**——Ollama 只有一份，會跟語音抽取搶同一個推論資源
...
NoteCommand command = parser.parse(...);
```

把快速路徑放進 `parse` 前面是直覺的做法，但它仍然**在限流之後**。
而 `LlmRateLimiter` 保護的是「只有一份」的 Ollama——讓一個
**完全不打 LLM** 的指令先跟它請額度，是把保護措施套在不需要保護的東西上。

後果不是浪費，是**使用者傳了幾則語音撞到上限之後，連按鈕都按不動了**，
為了一個根本用不到 Ollama 的操作。

拿掉這個順序，`QuickCommandTest` 會紅兩條（驗過）。

#### HELP 刻意不進 `NoteCommand.Action`

最直覺的做法是替說明加一個動作代號。但那個 enum 是
**餵給 LLM 的動作清單**——多一個 HELP，模型就會開始在不該的時候回傳它，
...
「列出」「還有什麼」那類說法留給 LLM。在它旁邊再造一個模糊比對的意圖判斷器，
兩邊會漸行漸遠，而不一致時很難查是哪一邊錯。**完全相符沒有這個問題**：
要嘛中，要嘛整句原封不動落回原本那條路。

#### 兩個決定加起來，差點做出一個沒人看得到的功能

- 選單**預設收合**（為了不擋住語音鍵——這個 bot 的主要輸入是語音）
- 歡迎訊息**不提選單**

這個功能要解的就是「不知道能做什麼」，結果它自己也需要被發現。
歡迎訊息現在會指向選單，而說明本文與歡迎訊息**共用同一段文字**——
各寫一份的話，改了其中一份就開始漂移，而漂移的症狀是
**使用者照著說明做、而系統的行為是另一套**。那比沒有說明更糟，因為他會相信它。

#### 後台建，不用 API

三顆按鈕都是靜態的，而圖會反覆調——那是視覺工作，在 UI 裡改快得多。
API 版每改一次圖都要重新上傳重新部署，對一個會反覆調的視覺元件是反向的。

代價是**設定不在版控裡**，補償是 `deploy/richmenu/`：圖、產生圖的程式、
以及一份「後台該填什麼」。兩個字串同時存在於後台與 `QuickCommand`，
而**後台改了字、程式碼沒跟著改的症狀不是壞掉**——按鈕會靜靜地掉回 LLM 解析，
多半仍然做對事，只是慢。不會有人發現。

順帶的好處是**點擊統計**：後台的分析頁看得到每顆按鈕被點幾次，可選 60 天。
API 建的沒有這個。第三顆「打開行事曆」價值存疑，就是靠這個決定去留。

### 34. 把判斷搬到程式碼，不等於搬到了正確的地方

**決策**：明確的相對星期（下禮拜一）由程式算好寫進 prompt；
同時保留「星期對不上就重試」的驗證層。

#### 起因是一個差七天的行程

2026-08-28（週五）說「下禮拜一早上去高雄」，抽成 **9/07**。正確是 8/31。

日曆表當時只標「今天」「明天」，其餘只有星期幾——而 **14 天裡每個星期幾都出現兩次**。
表格答得出「哪幾天是星期一」，答不出「**哪一個**是下週一」。

而這張表存在的理由，`NoteExtractor` 的註解自己寫著：

> 直接給日曆，而不是要模型自己算。實測光給「今天是星期五」還不夠

**它只做了一半，而剩下那一半正好是會錯的那一半。**

#### 診斷值得寫下來：不是理解力，是注意力

同一個模型：只給表 → 答對，而且說得出「查表查到標示為『下週一』的那一行」；
只給兩個候選 → 選對；**放進完整的抽取 prompt → 錯了四次以上**。

推理過程裡它編造了表格內容：

```
Table says: 2026-09-01（星期一）← 下週一.
```

表上寫的是 `2026-08-31（星期一）← 下週一`。而同一段推理裡，
它為了 title 那條規則來回糾結六次——**770 字的規則把日期擠掉了**。

#### 所以把它整個拿走

「下禮拜一」對應哪一天是**錄音時刻的全函數**，沒有判斷的餘地。
`RelativeDates` 直接算好寫進 prompt：「『下禮拜一』＝ 2026-08-31」。

> 這跟決策 33 拒絕「在 LLM 旁邊再造一個意圖判斷器」不衝突。差別在於
> **需要判斷的東西才該給模型**：意圖需要判斷，日曆不需要。
>
> 所以「星期五」刻意不解析——那可能是這週五也可能是下週五，
> 取決於今天星期幾與語境，**那才是判斷**。

#### 🔴 而這個「決定性」的解析器立刻把一個好答案弄壞了

第一版上線，eval 從 9/10 變成⋯⋯還是 9/10，只是換了一題失敗：
...
「記一**下**」的下跟「下週二」黏成「**下下週二**」，算出 8/25。
**而模型原本是對的**——是我把它弄壞的。

中文沒有詞界，而「一下」是極常見的動詞後綴。補了 `(?<!一)` 才對。

**這是這一項真正的內容**：把判斷從模型搬到程式碼，不會讓錯誤消失，
只會換一種錯法。程式碼的錯比較好測、比較好修、而且不會每次不一樣——
但它<b>不是免費的正確</b>。所以驗證層留著，現在它守的是<b>兩邊</b>。

#### 順帶炸出一個原本就在的 bug

重試耗盡時 `extract` 回 `null`，而 `@Cacheable` 沒有 `unless="#result == null"`。
Redis 不收 null，於是拋一個講快取設定的例外，**蓋掉 log 裡真正有用的那行**
「抽取連續 N 次驗證失敗」——症狀跟成因完全對不上。

驗證很少耗盡，所以這個洞放了很久沒人踩到。**加一道更嚴的檢查，
會讓既有的失敗路徑第一次被走到。**

#### 結果

PROMPT_VERSION v10 → v11，eval 從 9 題加到 10 題，連續三次 **10/10**。
