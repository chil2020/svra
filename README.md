...
> 這個專案的重點不是點子，而是把「一條 Python 腳本」（見 [legacy/app.py](legacy/app.py)）
> 重構成一套**事件驅動、多語言**的後端系統時，每一步的工程決策。

**為什麼做這個**：我在上一份工作負責金流平台的維運，反覆處理同一類問題——
高併發下交易狀態不一致。我能定位問題、能先止血，但根因是缺冪等與併發控制，
只有開發端能修。這個專案就是把那些「我看過它壞掉」的東西，親手實作一次。
