...
| | 限制 | 為什麼還沒修 |
|---|---|---|
| 🔴 | **引用失效版本的推播下指令，會回「已刪除」但什麼都沒發生。** `resolveQuoted()` 沒有檢查 `is_active`——對看不見的資料執行，然後回報成功 | 一行就能擋下來（加個 `active` 過濾），但正確的行為是什麼還沒想清楚：該說「那批已經被取代了」還是自動轉到現行版本？**成功訊息加零效果比報錯更難察覺**，所以值得想清楚再動 |
| 🔴 | **重抽會抹掉使用者的編輯。** `extractFor()` 從原始逐字稿重抽，刪過的項目會復活、改過的標題會變回原文 | outbox 重送那條路已經擋住了（見決策 18 的冪等守衛）。但「換模型重跑、兩版並存」那條路（決策 9）真的做出來時會再遇到——需要先決定編輯要怎麼跟著版本走 |
