...
**為什麼重構（2026）**：中間我去做了半年的 production 維運，負責金流平台，
反覆處理同一類問題——高併發下交易狀態不一致。我能定位、能先止血，
但根因是缺冪等與併發控制，只有開發端能修。

回頭看 2024 那版腳本，它犯的正是同一類錯：同步處理、沒有冪等、失敗就掉。
所以這次重構不是為了加功能，**是把「我親眼看過它壞掉」的東西，用對的方式做一次**。
產品想法沒變，變的是我知道該把哪些地方做對——
這個 repo 記錄的就是每一步的工程決策。
