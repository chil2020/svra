# ── 原始版本（2025），保留作為重構起點 ─────────────────────────────
# 這是 SVRA 的第一版：單一 Python 腳本，LINE 語音訊息 → faster-whisper 轉文字。
# 刻意原樣保存（含當時的縮排與結構問題），作為 README「從一條腳本到
# 事件驅動系統」重構故事的 before 對照。
# 2026-07-16：移除原本明碼寫死的 LINE 憑證，改讀環境變數（原憑證已作廢重發）。
# ──────────────────────────────────────────────────────────────────
